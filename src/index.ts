...
async function testListTools(endpoint: string): Promise<boolean> {
  ...
  try {
    ...
    if (!res.ok) {
      log(
        `Test request to ${endpoint}/mcp/list_tools failed with status ${res.status}`
      );
      return false;
    }
    ...
    log(
      `Received response from ${endpoint}/mcp/list_tools: ${currentResponse.substring(
        0,
        100
      )}...`
    );
...
async function findWorkingIDEEndpoint(): Promise<string> {
  ...
  log(`My Current Kamlesh Port ${process.env.IDE_PORT}`);
  ...
  if (process.env.IDE_PORT) {
    ...
    if (await testListTools(testEndpoint)) {
      ...
      log(
        `Specified IDE_PORT=${process.env.IDE_PORT} but it is not responding correctly.`
      );
      throw new Error(
        `Specified IDE_PORT=${process.env.IDE_PORT} but it is not responding correctly.`
      );
...
async function handleToolCall(
  ...
  try {
    log(
      `ENDPOINT: ${cachedEndpoint} | Tool name: ${name} | args: ${JSON.stringify(
        args
      )}`
    );
